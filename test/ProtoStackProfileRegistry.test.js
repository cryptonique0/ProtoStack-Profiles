...
describe("ProtoStackProfileRegistry", function () {
  let registry;
  let owner;
  let user1;
  let user2;
  ...
  describe("Profile Creation", function () {
    it("Should create a profile with valid username and fee", async function () {
      ...
      await expect(
        ...
        .withArgs(user1.address, username, ipfsHash, await ethers.provider.getBlock("latest").then(b => b.timestamp + 1));
  ...
  describe("Profile Updates", function () {
    beforeEach(async function () {
      await registry.connect(user1).createProfile("testuser", "QmOriginalHash", { value: CREATION_FEE });
    });
    ...
    it("Should update profile IPFS hash", async function () {
      ...
      await expect(
        registry.connect(user1).updateProfile(newHash)
  ...
  describe("Verification", function () {
    ...
    it("Should allow verifier to verify profile", async function () {
      await expect(
        registry.connect(verifier).verifyProfile(user1.address)
      ).to.emit(registry, "ProfileVerified");
    ...
    it("Should allow verifier to unverify profile", async function () {
      ...
      await expect(
        registry.connect(verifier).unverifyProfile(user1.address)
      ).to.emit(registry, "ProfileUnverified");
  ...
  describe("Admin Functions", function () {
    it("Should allow owner to add verifier", async function () {
      await expect(
        registry.addVerifier(verifier.address)
    ...
    it("Should allow owner to remove verifier", async function () {
      ...
      await expect(
        registry.removeVerifier(verifier.address)
