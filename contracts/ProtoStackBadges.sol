...
/**
 * @title ProtoVMBadges
 * @notice Achievement badge NFTs for ProtoVM profiles
 * @dev ERC1155 multi-token standard for badges
 */
contract ProtoVMBadges is ERC1155, ERC1155Supply, AccessControl, ReentrancyGuard {
  // ============ Roles ============
  ...
  event BadgeAwarded(address indexed recipient, uint256 indexed badgeId, uint256 timestamp);

  event BadgeRevoked(address indexed holder, uint256 indexed badgeId, uint256 timestamp);
   ...
   */
  function addCategory(string calldata _category) external onlyRole(BADGE_ADMIN_ROLE) {
   ...
   */
  function awardBadge(address _to, uint256 _badgeId) external onlyRole(MINTER_ROLE) nonReentrant {
   ...
   */
  function revokeBadge(address _from, uint256 _badgeId) external onlyRole(BADGE_ADMIN_ROLE) {
  ...
  function getUserBadges(address _user) external view returns (uint256[] memory, uint256[] memory) {
    uint256 count = 0;
