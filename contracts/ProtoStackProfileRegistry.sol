...
/**
 * @title ProtoVMProfileRegistry
 * @notice On-chain registry for ProtoVM user profiles
 * @dev Stores profile data hashes (IPFS CIDs) and verification status on-chain
 */
contract ProtoVMProfileRegistry is
  Initializable,
  OwnableUpgradeable,
  PausableUpgradeable,
  ...
  UUPSUpgradeable
{
  ...
  event ProfileCreated(address indexed owner, string username, string ipfsHash, uint256 timestamp);

  event ProfileUpdated(address indexed owner, string ipfsHash, uint256 timestamp);
  ...
  event ProfileVerified(address indexed owner, address indexed verifier, uint256 timestamp);

  event ProfileUnverified(address indexed owner, address indexed verifier, uint256 timestamp);
   ...
   */
  function verifyProfile(address _owner) external onlyVerifier profileExists(_owner) {
   ...
   * @param _owner Profile owner address
   */
  function unverifyProfile(address _owner) external onlyVerifier profileExists(_owner) {
  ...
  function getProfileByUsername(
    string calldata _username
  ) external view returns (Profile memory, address) {
  ...
  function getAllProfiles(
    uint256 _offset,
    uint256 _limit
  ) external view returns (Profile[] memory, address[] memory, uint256) {
